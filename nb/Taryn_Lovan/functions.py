...
################################################################################
#-------------------------------------------------------------------------------
def bin_data(data1, data2, bins1, bins2, label='', plot=False, density=False):
    '''
    ...
    bins1, bins2 : ndarray of length (n,)
        The bin edges
    ...
    n1, edges1 = np.histogram(data1, bins=bins1)
    ...
    n2, edges2 = np.histogram(data2, bins=bins2)
    ...
    if plot == True:
        plt.errorbar(x1, n1, yerr=dn1, fmt='.')        
        plt.errorbar(x2, n2, yerr=dn2, fmt='.')
        plt.xlabel(label)
        plt.ylabel('count') 
        plt.title("Binned Data")
        plt.show()
        
    return x1,x2,n1,n2,dn1,dn2
################################################################################
...
################################################################################
#-------------------------------------------------------------------------------
def logLjoint1_skew(params, m, n, x1, x2, peaks):
    """
    ...
    if peaks == 1:
        lambda1 = model_skew(pars, x1)
        lambda2 = s*model_skew(pars, x2)
    elif peaks == 2:
        lambda1 = mixturemodel_skew(pars, x1)
        lambda2 = s*mixturemodel_skew(pars,x2)
    elif peaks == 3:
        lambda1 = mixturemodel3_skew(pars, x1)
        lambda2 = s*mixturemodel3_skew(pars,x2)
    else:
    ...
    return np.sum(m*np.log(lambda1) - lambda1 - gammaln(m+1)) + \
           np.sum(n*np.log(lambda2) - lambda2 - gammaln(n+1))
################################################################################
...
################################################################################
#-------------------------------------------------------------------------------
def nlogLjoint1_skew(params, m, n, x1, x2, peaks):
    """Negative log-likelihood, for minimizers."""

    return -logLjoint1_skew(params, m, n, x1, x2, peaks)
################################################################################
...
################################################################################
#-------------------------------------------------------------------------------
def logLjoint2_skew(params, m, n, x1, x2, peaks):
    """Joint log-likelihood of the two data sets.
    ...
    peaks : list of integers
    	number of skew normal distributions in model
    ...
    if peaks[0] == 1:
        lambda1 = model_skew(params[:4], x1)
    elif peaks[0] == 2:
        lambda1 = mixturemodel_skew(params[:8], x1)
    elif peaks[0] == 3:
        lambda1 = mixturemodel3_skew(params[:12], x1)
    else:
        print('The mixture model for this many skew normals is not yet defined.')
        exit()
        
    if peaks[1] == 1:
        lambda2 = model_skew(params[4*peaks[0]:], x2)
    elif peaks[1] == 2:
        lambda2 = mixturemodel_skew(params[4*peaks[0]:], x2)
    elif peaks[1] == 3:
        lambda2 = mixturemodel3_skew(params[4*peaks[0]:], x2)
    else:
        print('The mixture model for this many skew normals is not yet defined.')
        exit()
...
################################################################################
#-------------------------------------------------------------------------------
def nlogLjoint2_skew(params, m, n, x1, x2, peaks):
    """Negative log likelihood, for minimizers."""

    return -logLjoint2_skew(params, m, n, x1, x2, peaks)
################################################################################
...
################################################################################
#-------------------------------------------------------------------------------
def Model_1_fit(bounds1, data1, data2, bins1, bins2, peaks, p0=None):
    '''
    ...
    x1, x2, n1, n2, dn1, dn2 = bin_data(data1, data2, bins1, bins2)
...
################################################################################
#-------------------------------------------------------------------------------
def Model_1_plot(params, data1, data2, bins1, bins2, peaks, xlabel_text='', title_text=''):
    '''
    ...
    if peaks == 1:
        m1 = model_skew(pars, x1)
        m2 = s*m1
    elif peaks == 2:
        m1 = mixturemodel_skew(pars, x1)
        m2 = s*m1
    elif peaks == 3:
        m1 = mixturemodel3_skew(pars, x1)
        m2 = s*m1
    else:
        print('The mixture model for this many skew normals is not yet defined.')
        exit()
    ...
    ep = ax.errorbar(x1, n1, yerr=dn1, fmt='k.')
    ax.plot(x1, m1, color=ep[0].get_color(), label='SDSS DR7')

    ep = ax.errorbar(x2, n2, yerr=dn2, fmt='r.')
    ax.plot(x1, m2, color=ep[0].get_color(), label='KOSS')
...
################################################################################
#-------------------------------------------------------------------------------
def Model_1_sampler(prior_, data1, data2, bins1, bins2, peaks, fname_suffix=''):
    '''
...
################################################################################
#-------------------------------------------------------------------------------
def Model_2_fit(bounds2, data1, data2, bins1, bins2, peaks, p0=None):
    '''
...
################################################################################
#-------------------------------------------------------------------------------
def Model_2_plot(params, data1, data2, bins1, bins2, peaks, xlabel_text='', title_text=''):
    '''
    ...
    pars1 = params[:4*peaks[0]]
    pars2 = params[4*peaks[0]:]

    if peaks[0] == 1:
        m1 = model_skew(pars1, x1)
    elif peaks[0] == 2:
        m1 = mixturemodel_skew(pars1, x1)
    elif peaks[0] == 3:
        m1 = mixturemodel3_skew(pars1, x1)
    else:
        print('The mixture model for this many skew normals is not yet defined.')
        exit()
        
    if peaks[1] == 1:
        m2 = model_skew(pars2, x1)
    elif peaks[1] == 2:
        m2 = mixturemodel_skew(pars2, x1)
    elif peaks[1] == 3:
        m2 = mixturemodel3_skew(pars2, x1)
    else:
        print('The mixture model for this many skew normals is not yet defined.')
        exit()
    ...
    ep = ax.errorbar(x1, n1, yerr=dn1, fmt='k.')
    ax.plot(x1, m1, color=ep[0].get_color(), label='Wall')

    ep = ax.errorbar(x2, n2, yerr=dn2, fmt='r.')
    ax.plot(x1, m2, color=ep[0].get_color(), label='Void')
...
################################################################################
#-------------------------------------------------------------------------------
def Model_2_sampler(prior_, data1, data2, bins1, bins2, peaks, fname_suffix=''):
    '''
    Run the dynesty nested sample to generate samples from the posterior PDF of 
    the M2 model.
