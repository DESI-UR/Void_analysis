...
c a huge program to do EVERYTHING that I want to do with voids in SDSS
      ...
      integer ne1, ne2, nl1, nl2
      integer ilambda, ieta, inout, nintest
      real pixsize, lamc, etac 
      integer vmask2sdss, vmask1sdss
      parameter (ne1 = 1602,ne2=800,nl1=5064,nl2=4576) ! DR4plus
      parameter (pixsize=0.025)
      real dec(ne1), ra(ne1)
      real dtor, rtod
      real*4 amask1(nl1,ne1),amask2(nl2,ne2)
      ...
      write(0,*) 'enter name of files'
c      read(*,'(A)') infile
      read(*,'(A)') out1
      read(*,'(A)') out2
      read(*,'(A)') out3
      write(*,*) 'enter the value of frac'
      read(*,*) frac
...
c      open(1,file=infile,status='unknown')
      open(1,file='../data/cbpdr7_102709.dat')

      open(12,file=out1,status='unknown')
      open(72,file=out2,status='unknown')
      open(9,file=out3,status='unknown')

c      open(3,file='SDSS_wall_20.5_0.107_d3un.dat',status='unknown')
c      open(2,file='SDSS_field_20.5_0.107_d3un.dat',status='unknown')

c      open(83,file='vollim_voidgals_20.5_0.107_100_300_d3un.dat')
c      open(84,file='maglim_voidgals_20.5_0.107_100_300_d3un.dat')

      write(*,*) 'here'
c---  Reading angular SF mask array
      open(7,file='sky1maskv.data025',form='unformatted')
      read(7)((amask1(ilambda,ieta),ilambda=1,nl1),ieta=1,ne1)
      close(7)

      open(7,file='sky2maskv.data025',form='unformatted')
...
c         call radeclameta(raf, decf, lamc, etac)
c         write(33,*) xin(i), yin(i), zin(i), raf, decf, r
c         write(34,*) raf, decf, lamc, etac
...
c            write(*,*) minsep1(i), minsep2(i), minsep3(i)
...
c                  if(ngal(i,j,k).EQ.0) write(*,*) 'found cell 1'
c     first of all check geometry - does cell lie in survey? 
...
c                  if(rac.LT.0) rac = rac + 360
c                  write(*,*) xcen, ycen, zcen, rac, decc, r
c need to make sure get the right ra
                  call radeclameta(rac,decc,lamc,etac)
                  inout = vmask2sdss(lamc,etac)+vmask1sdss(lamc,etac)
                  if (inout.EQ.1) then
                     if(etac.GT.0) then
                        ieta = int((41.25-etac)/pixsize)+1
                        ilambda = int((lamc+63.1)/pixsize)+1
                        compl = amask1(ilambda,ieta)
                     else if(etac.LT.0) then
                        ieta = int((-16.25-etac)/pixsize)+1 
                        ilambda = int((lamc+62.8)/pixsize)+1 
                        compl = amask2(ilambda,ieta)
                     end if
                     if(compl.GT.0.9) then ! ok in survey
c     this is an empty cell and worth growing a void from it
c                        write(*,*) xcen, ycen, zcen, compl, ilambda,ieta
                        ioff = 0
                        nposs = 0
                        do while (nposs.LT.1)
                           ioff = ioff + 1
...
c     check this is still in the survey
                     call radeclameta(ra2,dec2,lamc,etac)
                     inout = vmask2sdss(lamc,etac)+vmask1sdss(lamc,etac)
                     if(inout.EQ.1) then
...
c                                 write(*,*) xcen4, ycen4, zcen4
c                                 write(*,*) sep, rad, rad2, rad3
c                                 write(*,*) 'values'
...
c check this is still in the survey
                                 call radeclameta(ra4,dec4,lamc,etac)
                   inout = vmask2sdss(lamc,etac)+vmask1sdss(lamc,etac)
                                 if(inout.EQ.1) then
                                    if(etac.GT.0) then
                                      ieta = int((41.25-etac)/pixsize)+1
                    ...
                    if(compl.GT.0.9.AND.r4.LT.dmax.AND.r4.GT.dmin) then ! ok in survey
c                              write(*,*) xcen4,ycen4,zcen4,rad,ra4,dec4
                              diff = abs(rad2 - rad3)
             if(diff.GT.0.1) write(*,*) 'problem', sep, rad, rad2, rad3
             if(diff.GT.0.1) write(*,*) 'problem', ra4, dec4             
c             write(*,*) xcen4, ycen4, zcen4
c             write(*,*) minx1, minx2, dotu1, dotu2

c             write(*,*) xw(k1g), yw(k1g), zw(k1g)
c             write(*,*) xw(k2g), yw(k2g), zw(k2g)
c             write(*,*) xw(k3g), yw(k3g), zw(k3g)
c             write(*,*) xw(k4g), yw(k4g), zw(k4g)
...
c**** check the ra and pi thing here
               ...
               decf = decf*180./pi
c     check this is still in the survey
               call radeclameta(raf,decf,lamc,etac)
               inout = vmask2sdss(lamc,etac)+vmask1sdss(lamc,etac)
               if(inout.EQ.1) then
                  if(etac.GT.0) then
                     ieta = int((41.25-etac)/pixsize)+1
...
c         write(*,*) i, rai,deci,lambda,eta,zi,rabsmagi,uri
         inout = vmask2sdss(lambda,eta) + vmask1sdss(lambda,eta)
c check in the boundary we are going to use using inout
         if(inout.GT.0.5) then
            zplus1 = zi + 1
            ...
            if(r.GT.dmin.AND.r.LT.dmax) then
               ...
               write(52,*) xf(ngb), yf(ngb), zf(ngb), rabsmagi
            end if
         end if
      end do
 87   write(*,*) ngb
...
c-------------------------------------------------------------------
      ...
      subroutine radeclameta(ra,dec,lam,eta)
      real ra, dec, lam, eta, slambda, clambda, pi
      real dtor, rtod
      ...
      lam = asin(cos(dec*dtor)*sin((ra-185.)*dtor))
      eta= atan(tan(dec*dtor)/sin((ra-95.)*dtor))-32.5*dtor
      slambda = sin((ra-185)*dtor)*cos(dec*dtor)
      clambda = sin(dec*dtor)/sin(eta+32.5*dtor)

         
      if(slambda.gt.0.)then
         if(clambda.gt.0.)then
            lam = lam
         else
            lam = pi-abs(lam)
         end if
...
c---  SDSS [outer boundary] mask function for the (upper) Area 1 
c---  Inputs are in [degree] unit; LSS-Dr4plus void0
      function vmask1sdss(lambda,eta)
      integer m, vmask1sdss
      real lambda,eta,x,y
      ...
      m = 0
c---  Zone 1
      if(x.ge.-47.1.and.x.lt.-28.0 .and.y.ge.38.75.and.y.lt.41.25)m = 1
      if(x.ge.-49.8.and.x.lt.-27.5 .and.y.ge.36.25.and.y.lt.38.75)m = 1
c---  Zone 2
      if(x.ge.-50.0.and.x.lt.48.317.and.y.ge.33.75.and.y.lt.36.25)m = 1
c---  Zone 3
      if(x.ge.-48.282.and.x.lt.59.7.and.y.ge.31.25.and.y.lt.33.75)m = 1
c---  Zone 4
      if(x.ge.-53.906.and.x.lt.60.4.and.y.ge.28.75.and.y.lt.31.25)m = 1
c---  Zone 5
      if(x.ge.-56.8. and.x.lt.53.8 .and.y.ge.26.25.and.y.lt.28.75)m = 1
c---  Zone 6
      if(x.ge.-58.0 .and.x.lt.53.8 .and.y.ge.23.75.and.y.lt.26.25)m = 1
c---  Zone 7
      if(x.ge.-53.906.and.x.lt.62.36.and.y.ge.21.25.and.y.lt.23.75)m= 1
c---  Zone 8
      if(x.ge.-59.8 .and.x.lt.63.0.and.y.ge.18.75.and.y.lt.21.25)m = 1
c---  Zone 9
      if(x.ge.-60.6 .and.x.lt.63.2.and.y.ge.16.25.and.y.lt.18.75)m = 1
c---  Zone 10
      if(x.ge.-60.6 .and.x.lt.63.3.and.y.ge.13.75.and.y.lt.16.75)m = 1
c---  Zone 11
      if(x.ge.-61.6 .and.x.lt.63.5.and.y.ge.11.25.and.y.lt.13.75)m = 1
c---  Zone 12 
      if(x.ge.-63.1 .and.x.lt.63.5.and.y.ge. 8.75.and.y.lt.11.25)m = 1
c---  Zone 13
      if(x.ge.-63.1 .and.x.lt.58.18.and.y.ge. 6.25.and.y.lt.8.75)m = 1
c---  Zone 14
      if(x.ge.-63.1 .and.x.lt.5.32.and.y.ge. 2.2.and.y.lt.6.25)m = 1
c---  Zone 15
      if(x.ge.-63.1 .and.x.lt.-18.9.and.y.ge. 1.2.and.y.lt.2.2)m = 1
      vmask1sdss = m
      return
      end

c---  SDSS [outer boundary] mask function for the (lower) Area 2
c---  Inputs are in [degree] unit
      function vmask2sdss(lambda,eta)
      integer m, vmask2sdss
      real lambda,eta,x,y
      ...
      m = 0
c---  Zone 1
      if(x.ge.-62.8.and.x.lt.15.0.and.y.ge.-18.75.and.y.lt.-16.25)m = 1
      if(x.ge.-62.8.and.x.lt.15.0.and.y.ge.-21.25.and.y.lt.-18.75)m = 1
c---  Zone 2
      if(x.ge.-57.1.and.x.lt.5.0.and.y.ge.-23.75.and.y.lt.-21.25)m = 1
c---  Zone 3
c      if(x.ge.-61.9.and.x.lt.5.0.and.y.ge.-26.25.and.y.lt.-23.75)m = 1
      if(x.ge.-57.1.and.x.lt.5.0.and.y.ge.-26.25.and.y.lt.-23.75)m = 1
c---  Zone 4
      if(x.ge.-55.2.and.x.lt.43.38.and.y.ge.-28.75.and.y.lt.-26.25)m=1
c---  Zone 5
      if(x.ge.-54.7.and.x.lt.43.38.and.y.ge.-31.25.and.y.lt.-28.75)m=1
c---  Zone 6
      if(x.ge.-54.7.and.x.lt.51.6.and.y.ge.-33.75.and.y.lt.-31.25)m = 1
c---  Zone 7
      if(x.ge.-14.2.and.x.lt.51.6.and.y.ge.-36.25.and.y.lt.-33.75)m = 1
